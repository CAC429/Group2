...
class DataGridUI:
    ...
    def Update_UI(Self):
        ...
        try:
            ...
            # Update Train_Bauds assuming list of lists: [Baud1, Baud2, Baud3, Baud4]
            data["Train_Bauds"] = {
                "1-28-Baud1=": Train_Bauds[0],
                "29-76-Baud2=": Train_Bauds[1],
                "77-100-Baud3=": Train_Bauds[2],
                "100-150-Baud4=": Train_Bauds[3]
            }

            # Write back to JSON
            with open("PLC_OUTPUTS_Baud_Train_Instance.json", "w") as file:
        ...
        # Define file paths
        original_file = "PLC_OUTPUTS.json"
        backup_file = "PLC_OUTPUTS_backup.json"
        ...
        if os.path.getsize("PLC_OUTPUTS.json") != 0:
            # Read PLC_OUTPUTS.json
            try:
                with open("PLC_OUTPUTS.json", "r") as file:
                    outputs = json.load(file)
        ...
        # Prepare the data to write
        outputs = {
            "Suggested_Speed": Suggested_Speed_Out,
            "Suggested_Authority": Suggested_Authority_Out,
            "Occupancy": Occupancy_Out,
            "Track_Failure": Track_Failure_Out,
            "Light_Control": Light_Control_Out,
            "Actual_Switch_Position": Actual_Switch_Position_Out,
        ...
        #Self.Speed_Data = np.round(np.array(Temp_Speed) * 0.621371, 2)
        #Self.Authority_Data = np.round(np.array(Temp_Authority) * 3.28084, 2)
