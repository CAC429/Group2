...
def send_to_pi(json_data):
    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        
        try:
            ssh.connect('10.5.19.8', username='ethannam', password='password', timeout=5)
            ...
            print(f"Error connecting to Pi: {str(e)}")
            return

        try:
            cmd = f'echo \'{json.dumps(json_data)}\' > /tmp/train_hw_pipe'
            stdin, stdout, stderr = ssh.exec_command(cmd)
            
            # Check for errors
            if stderr.read():
                print("Error executing command on Pi")
            
            ssh.close()
            print(f"Sent to pi: {json_data}")
            
        except Exception as e:
            print(f"Error sending command to Pi: {str(e)}")
            ssh.close()
    ...
    def write_outputs(self, filename, **kwargs):
        try:
            ...
            if self.current_train_id == "1":
                json_data = {
                    "leds": {
                        "service_brake": data["Service Brake"],
                        "emergency_brake": data["Emergency Brake"],
                        "left_door": data["Left Door"],
                        "right_door": data["Right Door"],
                        "cabin_light": data["Cabin Lights"],
